...
def setup_state(state):
    ...
    if "gemini_api_key" not in state:
        state["gemini_api_key"] = os.getenv("GOOGLE_API_KEY", "") or os.getenv(
            "GEMINI_API_KEY", ""
        )
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
            ...
            with gr.Column():
                ...
                planner_model = gr.Dropdown(
                    ...
                    choices=[
                        ...
                        # Gemini models (free tier with vision)
    ...
    def update_planner_model(model_selection, state):
        ...
        # Gemini models (free tier with vision support)
            ...
            provider_interactive = False
            api_key_interactive = True
            api_key_placeholder = "Gemini API key (free at aistudio.google.com/apikey)"
            actor_model_choices = ["ShowUI", "UI-TARS"]
            actor_model_value = "ShowUI"
            actor_model_interactive = True
            api_key_type = "password"
            # Extract the actual model name (remove " (Gemini)" suffix)
            state["planner_model"] = model_selection.replace(" (Gemini)", "")
